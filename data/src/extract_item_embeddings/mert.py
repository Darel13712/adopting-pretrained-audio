...
def embed_batch(model, batch):
    with torch.no_grad():
        ...
        emb = model(batch).last_hidden_state.cpu().numpy()
        return emb
...
model = AutoModel.from_pretrained("m-a-p/MERT-v1-330M", trust_remote_code=True).cuda()
e = embed(model, track_paths, 16)
np.save('embeddings/mert.npy', e)
